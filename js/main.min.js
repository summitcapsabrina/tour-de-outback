...
function initWeather() {
  ...
  var todayHigh = null; // today's forecast high for first card var todayLow=null;'s forecast low for first card
  ...
  function buildForecast(periods) {
    ...
    while (i < periods.length && days.length < 3) {
      ...
      // Store today's high/low from the first card if(days.length===0){todayHigh=day.high;todayLow=day.low;}days.push(day);}var html='<div class="forecast-header">3-Day Forecast — Lakeview, OR</div><div class="forecast-cards">';for(var j=0;j<days.length;j++){var d=days[j];html+='<div class="forecast-card">'+'<div class="forecast-day">'+d.name+'</div>'+'<div class="forecast-date">'+d.date+'</div>'+'<div class="forecast-card-icon">'+d.icon+'</div>'+'<div class="forecast-temps">'+(d.high!==null?'<span class="forecast-high">High '+d.high+'°F</span>':'')+(d.low!==null?'<span class="forecast-low">Low '+d.low+'°F</span>':'')+'</div>'+(d.precip?'<div class="forecast-precip">'+d.precip+'</div>':'')+buildForecastCompass(d.windDir,d.windSpeed)+'<div class="forecast-desc">'+d.desc+'</div>'+'</div>';}html+='</div>';return html;}function syncFirstCard(){var firstCard=widget.querySelector('.forecast-card');if(!firstCard)return;"Now XX°F" if(currentObsTemp!==null){var dayEl=firstCard.querySelector('.forecast-day');if(dayEl)dayEl.innerHTML='Now '+currentObsTemp+'°F';}if(currentObsIcon){var iconEl=firstCard.querySelector('.forecast-card-icon');if(iconEl)iconEl.textContent=currentObsIcon;}'s forecast high/low below icon (only if we have forecast data)
  ...
  fetch('https:', { headers: nwsHeaders })
...
document.addEventListener('DOMContentLoaded',()=>{initCountdown();initMobileNav();initNavScroll();initFAQ();setActiveNav();initNewsletter();initRegisterTracking();initWeather();initRouteSwitch();});